...
_GROUNDING_RULES = (
    ...
    "Propose specific, actionable recommendations. State concrete quantities, divisions, or actions ONLY if they are directly present in the source data or calculated from it. If the exact quantity cannot be derived from the data, recommend the qualitative action without inventing numerical targets.\n"
    "RECOMMENDATIONS MUST TIE TO EVIDENCE: Every recommendation must directly connect the data evidence to the proposed action. Do not write vague recommendations (e.g. 'Review inventory strategy', 'Increase reorder level', 'Review sales strategy'). Instead, formulate recommendations by linking the metric value and action together, for example: 'Petrol inventory covers only 14 days of demand while sales have increased by 23.1%; review reorder thresholds to maintain a higher coverage level.' If a precise quantitative target cannot be derived from the data, recommend a qualitative operational action rather than inventing numerical goals.\n"
    ...
    "\nANALYTICAL DEPTH & EVIDENCE-BASED ROOT CAUSES (for business/financial/operational reports):\n"
    "Perform strict evidence-backed deductive causal reasoning. For every key finding, explain WHY it happened using concrete data evidence (e.g., variance of X% in budget, or concentration of Y% of delays in specific projects, or specific experience levels matching specific delays).\n"
    "Prohibit making generic, unsupported assumptions (like concluding 'inefficient resource allocation' or 'poor scheduling' just because there are delays) without presenting the actual data metrics from the report that lead to this conclusion. Trace issues to operational bottlenecks, replenishment lags, demand increases, competitor positioning, or capacity gaps.\n"
    "CAUSAL HYPOTHESIS LABELING: Clearly label inferred explanations or hypothesized root causes as hypotheses (e.g. using terms like 'may have contributed to', 'suggests a potential link', 'a possible hypothesis is...'). Reserve definitive statements ONLY for conclusions directly supported by the data. Do NOT over-interpret stable, healthy, or expected conditions as problems or operational failures; verify if the data actually shows a variance before claiming an issue exists.\n"
    "Structure logic: Observed Metric -> Cross-File Evidence -> Causal Hypothesis supported by data -> Business Impact -> Recommendation.\n"
...
_REPORT_SPECIFIC_RULES = (
    ...
    "Structure each key finding exactly as follows:\n"
    "  ### Finding: [Short Descriptive Finding Title]\n"
    "  * **Observation**: [what happened]\n"
    "  * **Evidence**: [exact numbers, values, and trends from the data]\n"
    "  * **Why it matters**: [operational impact, causal root cause e.g., replenishment lag, operational bottleneck, or competitor positioning]\n"
    "  * **Recommended action**: [specific qualitative next step, tied directly to the evidence]\n"
    "  * **Explainability Metadata**:\n"
    "    * Finding ID: [INS-XXX]\n"
    "    * Finding Confidence: [Critical/High/Medium/Low] (Confidence Score)\n"
    "    * Evidence Source: [Supporting files, columns, and metrics]\n"
...
_CROSS_FILE_RULES = (
    ...
    "- CROSS-DOCUMENT TIMELINE COMPARISONS: When sequential trends exist, compare values across any sequential business state (including versions, reporting periods, project phases, releases, financial years, or operational milestones) rather than limiting comparisons strictly to dates. Trace changes systematically across these sequences.\n"
    "- DETERMINISTIC CONFIDENCE & EXPLAINABILITY:\n"
    "  For each major finding, you MUST output a metadata box exactly displaying the finding ID, deterministic confidence score, and exact file/metric evidence sources, verbatim from the DataFusion context (do NOT invent or alter these confidence values or IDs):\n"
    "  ```\n"
    "  * Finding ID: [INS-XXX]\n"
    "  * Finding Confidence: [Critical/High/Medium/Low] (Confidence Score)\n"
    "  * Evidence: [Supporting files, columns, and metrics]\n"
    "  ```\n"
)
...
_DOCTYPE_OUTPUT_RULES = {
    ...
    "business_report": (
        ...
        "  ## KPI Dashboard  (You MUST output an executive performance-dimension table structured exactly as follows:\n"
        "    | Performance Dimension | Metric / Segment | Value / Trend | Highlighted Insight & Risk Level |\n"
        "    Highlight the following 5 dimensions in this table, rather than simple averages across departments:\n"
        "    1. Highest/Best Performer (the segment/metric with optimal results)\n"
        "    2. Lowest/Worst Performer (the segment/metric showing major gaps)\n"
        "    3. Largest Variance / Volatility (highest fluctuation or deviation)\n"
        "    4. Fastest Growth / Trend (strongest upward/downward trend)\n"
        "    5. Highest Operational Risk (safety, compliance, or severe delay factors)\n"
        "    Add Target/Variance columns only if targets exist in the data.)\n"
